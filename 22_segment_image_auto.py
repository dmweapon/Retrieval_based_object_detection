...
def main():
    ...
    class_dirs = sorted([d.name for d in source_base_dir.iterdir() if d.is_dir()])
    print("\n[3/4] 처리할 클래스를 선택하세요.")
    for i, class_dir_name in enumerate(class_dirs, 1):
        print(f"[{i}] {class_dir_name}")
    
    try:
        class_choice = int(input("클래스 번호 선택: ")) - 1
        class_name = class_dirs[class_choice]
    ...
    # 4. 세그멘테이션 실행
    print("\n[4/4] 자동 세그멘테이션을 시작합니다.")
    print(f"Source: {input_dir}")
    print(f"Target: {output_dir}")

    image_paths = sorted(list(input_dir.glob("*.jpg")) + list(input_dir.glob("*.jpeg")))
    
    if not image_paths:
        print(f"⚠️ 처리할 이미지가 없습니다: {input_dir}")
        sys.exit(0)

    print(f"\n▶️ '{class_name}' 클래스의 {len(image_paths)}개 이미지 처리 중...")

    for image_path in tqdm(image_paths, desc=f"Processing {class_name}"):
        try:
            image_bgr = cv2.imread(str(image_path))
            if image_bgr is None:
                print(f"⚠️ 이미지를 읽을 수 없습니다: {image_path.name}")
                continue
            ...
            print(f"❌ 처리 중 오류 발생 ({image_path.name}): {e}")

    print(f"\n✅ 완료! 결과가 다음 경로에 저장되었습니다: {output_dir}")
